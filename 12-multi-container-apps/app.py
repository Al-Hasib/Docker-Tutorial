...
Same Postgres-backed storage as topic 11, plus a Redis cache in front of
GET /notes (cache-aside pattern, see cache.py). The response now includes a
"source" field ("cache" or "database") so it's obvious on screen which path
served the request.
"""
